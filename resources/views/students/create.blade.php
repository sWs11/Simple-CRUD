                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('students.store') }}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <select class="form-control custom-select @error('group_id') is-invalid @enderror" name="group_id" id="group_id">
                                        <option value="">Select group</option>
                                        @foreach($courses as $course => $groups)
                                            <optgroup label="Course {{ $course }}">
                                                @foreach($groups as $group)
                                                    <option value="{{ $group->id }}">{{ $group->number }} ({{ $group->faculty_name }})</option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
                                    </select>
